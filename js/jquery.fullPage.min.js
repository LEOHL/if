    ...
    b.fn.fullpage = function (c) {
        ...
        b(document).on("touchmove", function (a) {
            if (c.autoScrolling && B) {
                ...
                if (!k && !s)
                    if (E = a.touches[0].pageY, y = a.touches[0].pageX, e.find(".slides").length && Math.abs(x - y) > Math.abs(D - E)) x > y ? e.find(".controlArrow.next").trigger("click") : x < y && e.find(".controlArrow.prev").trigger("click");
